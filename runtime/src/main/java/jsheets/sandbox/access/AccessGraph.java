...
public final class AccessGraph {
  ...
  private static Collection<AccessGraphNode> findClosestMatchOrMethods(
    AccessGraphNode node,
    String lastKey
  ) {
    var children = node.children()
      .filter(child -> child.key().equals(lastKey))
      .toList();
  ...
  public boolean isMethodPermitted(MethodSignature signature) {
    var key = AccessKey.slashSeparated(signature.formatWithoutTypes());
    var matches = findClosestMatch(key);
    var access = switch (matches.size()) {
      case 0 -> Access.Denied;
      case 1 -> matches.iterator().next().access();
      default -> findBestMatch(signature, matches).map(AccessGraphNode::access);
    };
    return access.equals(Access.Permitted);
  }

  private Optional<AccessGraphNode> findBestMatch(
